/*
 * qrcode-lib.js
 * Minimal, dependency-free QR Code generator for the browser.
 * Based on the public-domain QR algorithm (Reed-Solomon error correction,
 * as popularized by Kazuhiko Arase's qrcode-generator library).
 * Supports byte-mode encoding, auto version selection, and EC levels L/M/Q/H.
 */
(function (global) {
  'use strict';
  ...
  QR8BitByte.toUTF8Bytes = function (s) {
    ...
    for (var i = 0; i < s.length; i++) {
      ...
      if (code > 0xFFFF) i++; // consume surrogate pair
      if (code < 0x80) {
        bytes.push(code);
      } else if (code < 0x800) {
  ...
  // ---------- Auto type number selection ----------
  function getMinimumTypeNumber(sentence, errorCorrectLevel) {
    for (var typeNumber = 1; typeNumber <= 40; typeNumber++) {
      try {
        var testModel = new QRCodeModel(typeNumber, errorCorrectLevel);
        testModel.addData(sentence);
        testModel.make();
        return typeNumber;
      } catch (e) { continue; }
    }
  ...
  // ---------- Public API: draws to a <canvas> ----------
  function QRCodeCanvas(el, options) {
    options = options || {};
    ...
    this.correctLevel = options.correctLevel !== undefined ? options.correctLevel : QRErrorCorrectLevel.M;
    this.el = (typeof el === 'string') ? document.getElementById(el) : el;
    this._draw();
  }
  QRCodeCanvas.prototype._draw = function () {
    this.el.innerHTML = '';
    var errorLevel = this.correctLevel;
    var typeNumber = getMinimumTypeNumber(this.text, errorLevel);
    var model = new QRCodeModel(typeNumber, errorLevel);
    model.addData(this.text);
    model.make();
    ...
    var tileW = this.width / moduleCount;
    var tileH = this.height / moduleCount;
    ...
    for (var row = 0; row < moduleCount; row++) {
      for (var col = 0; col < moduleCount; col++) {
        ctx.fillStyle = model.isDark(row, col) ? this.colorDark : this.colorLight;
        var x = Math.round(col * tileW), y = Math.round(row * tileH);
        var w = Math.ceil((col + 1) * tileW) - x, h = Math.ceil((row + 1) * tileH) - y;
  ...
  QRCodeCanvas.prototype.toDataURL = function (type) { return this._canvas.toDataURL(type || 'image/png'); };

  global.QRCodeCanvas = QRCodeCanvas;
  global.QRErrorCorrectLevel = QRErrorCorrectLevel;

  // Expose for Node-based testing/verification
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { QRCodeModel: QRCodeModel, QRErrorCorrectLevel: QRErrorCorrectLevel, getMinimumTypeNumber: getMinimumTypeNumber };
  }
